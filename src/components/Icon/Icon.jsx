...
// 1. Definisikan semua path SVG Anda di dalam satu objek
// Kuncinya adalah nama ikon, nilainya adalah JSX dari SVG itu sendiri.
const iconPaths = {
  folder: (
...
// 2. Buat komponen <Icon> yang serbaguna
const Icon = ({ name, ...props }) => {
  // Ambil SVG berdasarkan nama yang diberikan
  const IconSvg = iconPaths[name];

  // Jika nama ikon tidak ditemukan, jangan render apa pun
  if (!IconSvg) {
    return null;
  }

  // Duplikasi elemen SVG dan tambahkan properti lain (seperti className)
  // Ini membuat komponen kita sangat fleksibel
  return React.cloneElement(IconSvg, props);
};
